...
def config(dataset):
    ...
    if dataset == 'AffectNet-7':
        ...
        train_pd = pd.read_csv("../datas/AffectNet/training-cls7-v6.txt", sep=" ", header=None,
    ...
    if dataset == 'FERPlus+RAF-DB':
        ...
        train_pd = pd.read_csv("../datas/FERPlus+RAF-DB/ferplus_raf_db_training.txt", sep=" ", header=None,
                               names=['ImageName', 'label'], engine='python')
        test_pd = pd.read_csv("../datas/FERPlus+RAF-DB/ferplus_raf_db_test.txt", sep=" ", header=None,
                              names=['ImageName', 'label'], engine='python')
        cls_num = 8
...
class Dataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.train:
            ...
            negative_image, neg_label = self.fetch_negative(self.num_negative, label, self.image_names[item])
            negative_image2 = self.fetch_positive(self.num_positive, neg_label, self.image_names[item])
    ...
    def fetch_negative(self, num_negative, label, path):
        other_img_info = self.pd_file[(self.pd_file.label != label) & (self.pd_file.ImageName != path)]
